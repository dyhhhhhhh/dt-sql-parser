...
createTableStatement
    : CREATE EXTERNAL? TABLE (IF NOT EXISTS)? qualifiedName LEFT_PAREN columnDesc (
        COMMA columnDesc
    )* (COMMA indexDesc)* RIGHT_PAREN engineDesc? charsetDesc? keyDesc? withRowAccessPolicy* comment? partitionDesc? distributionDesc? orderByDesc?
        rollupDesc? properties? extProperties?
    ;
...
createTemporaryTableStatement
    : CREATE TEMPORARY TABLE qualifiedName queryStatement
    ;

createTableAsSelectStatement
    : CREATE TABLE (IF NOT EXISTS)? qualifiedName (
        LEFT_PAREN identifier (COMMA identifier)* RIGHT_PAREN
    )? keyDesc? comment? partitionDesc? distributionDesc? properties? AS queryStatement
    ;
...
createIndexStatement
    : CREATE INDEX indexName=identifier ON qualifiedName identifierList indexType? comment?
    ;
...
refreshTableStatement
    : REFRESH EXTERNAL TABLE qualifiedName (
        PARTITION LEFT_PAREN string (COMMA string)* RIGHT_PAREN
    )?
    ;

showAlterStatement
    : SHOW ALTER TABLE (COLUMN | ROLLUP) ((FROM | IN) db=qualifiedName)? (WHERE expression)? (
...
cancelAlterTableStatement
    : CANCEL ALTER TABLE (COLUMN | ROLLUP)? FROM qualifiedName (
...
createViewStatement
    ...
    )? withRowAccessPolicy* comment? AS queryStatement
    ;
...
createMaterializedViewStatement
    : CREATE MATERIALIZED VIEW (IF NOT EXISTS)? mvName=qualifiedName (
        LEFT_PAREN columnNameWithComment (COMMA columnNameWithComment)* RIGHT_PAREN
    )? withRowAccessPolicy* comment? materializedViewDesc* AS queryStatement
    ;
...
applyMaskingPolicyClause
    : MODIFY COLUMN columnName=identifier SET MASKING POLICY policyName=qualifiedName (
...
// ------------------------------------------- Routine Statement -----------------------------------------------------------
createRoutineLoadStatement
    : CREATE ROUTINE LOAD (db=qualifiedName DOT)? name=identifier ON table=qualifiedName loadPropertiesExpr jobProperties? FROM source=identifier
        dataSourceProperties?
    ;

alterRoutineLoadStatement
    : ALTER ROUTINE LOAD FOR (db=qualifiedName DOT)? name=identifier loadPropertiesExpr jobProperties? dataSource?
    ;
...
dataDesc
    ...
    )? (ROWS TERMINATED BY rowSep=string)? format=fileFormat? (formatPropsField=formatProps)? colList=columnAliases? (
...
formatProps
    : LEFT_PAREN (SKIP_HEADER EQ INTEGER_VALUE)? (TRIM_SPACE EQ booleanValue)? (
        ENCLOSE EQ encloseCharacter=string
    )? (ESCAPE EQ escapeCharacter=string)? RIGHT_PAREN
    ;
...
showTabletStatement
    ...
    | SHOW TABLET FROM qualifiedName partitionNames? (WHERE expression)? (
        ORDER BY sortItem (COMMA sortItem)*
    )? (limitElement)?
    ;
...
createMaskingPolicyStatement
    : CREATE (OR REPLACE)? MASKING POLICY (IF NOT EXISTS)? policyName=qualifiedName AS LEFT_PAREN policySignature (
        COMMA policySignature
    )* RIGHT_PAREN RETURNS type ARROW expression comment?
    ;
...
createRowAccessPolicyStatement
    : CREATE (OR REPLACE)? ROW ACCESS POLICY (IF NOT EXISTS)? policyName=qualifiedName AS LEFT_PAREN policySignature (
        COMMA policySignature
    )* RIGHT_PAREN RETURNS BOOLEAN ARROW expression comment?
    ;
...
createRepositoryStatement
    : CREATE (READ ONLY)? REPOSITORY repoName=identifier WITH BROKER brokerName=identifierOrString? ON LOCATION location=string (
        PROPERTIES propertyList
    )?
    ;
...
querySpecification
    : SELECT setVarHint* setQuantifier? selectItem (COMMA selectItem)* fromClause (
...
relationPrimary
    : qualifiedName temporalClause? partitionNames? tabletList? (AS? alias=identifier)? bracketHint? # tableAtom
...
over
    ...
    )? (ORDER BY sortItem (COMMA sortItem)*)? RIGHT_PAREN
    ;
